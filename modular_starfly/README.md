# README.md
The modularization handbook - Starfly style, v0.3

Adapted from: [Modular Skyrat](https://github.com/Sea-of-Lost-Souls/Tannhauser-Gate/blob/master/modular_skyrat/readme.md)
...
**If it's a shiptest-specific tweak or bugfix, first course of action should be an attempt to discuss and PR it upstream, instead of needlessly modularizing it here.**
...
And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_starfly/modules/shuttle_toggle`
...
We will no longer have _starfly map versions.
...
- ***Example:*** You're adding a new lavaland mob.
  ...
  First of all you create your modular folder. E.g. `modular_starfly/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_starfly/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_starfly/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_starfly/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_starfly/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_starfly/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes STARFLY edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. STARFLY edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  //STARFLY EDIT ADDITION BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  ```byond
  //STARFLY EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  /*
  ...
  */
  ...
  ```byond
  //STARFLY EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Moved to modular_starfly/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  //STARFLY EDIT CHANGE BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - STARFLY EDIT - ORIGINAL
...
That folder is **`code/__DEFINES/~starfly_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_starfly/modules/example_module/code/disease_mob.dm
- Bad: /modular_starfly/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_starfly/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_starfly/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for STARFLY UIs.
...
You can do both `// STARFLY EDIT` and `/* STARFLY EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	...
	icon="rat" // STARFLY EDIT ADDITION
</Button>
```
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// STARFLY EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
```js
// THIS IS A STARFLY UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a STARFLY edit comment in a modular TGUI file.
